...
def _move_to_next_word_paragraph_or_eof(text_cursor, expand: bool, cross_empty: bool) -> bool:
...
def _word_motion_once_forward(text_cursor, expand: bool, spec) -> bool:
    ...
    # If cursor is past paragraph or document ends.
    if offset >= length:
        return _move_to_next_word_paragraph_or_eof(text_cursor, expand, cross_empty)
    ...
    move = next_offset
    # In expand mode, e/E should include the last character of the word.
    # w/W keeps the caret before the first character of the next word.
